...
Потребно е да се развие веб апликација за управување со трошоци.

## Функционални барања

- **(20 поени)** Потребно е на патеките `/` и `/expenses` да прикажете листа од сите креирани трошоци со
  користење на темплејтот `list.html`. Дополнително тука треба да овозможите пагинација на трошоците.
    - Имплементацијата на оваа функционалност може да ја проверите со тестовите `test_list_10pt`
      и `test_pagination_10pt`.

- **(20 поени)** Потребно е да се имплементира додавање на трошок. При клик на копчето **Add new expense** од
  темплејтот `list.html`, потребно е да се прикаже темплејтот `form.html` на патеката `/expenses/add`, каде при клик
  на **Submit** ќе се креира и запише нов трошок во базата на податоци. Потоа треба да се прикаже
  страната `/expenses`.
    - Имплементацијата на оваа функционалност може да ја проверите со тестовите `test_create_10pt`
      и `test_create_mvc_10pt`.

- **(5 поени)** Потребно е да се имплементира бришење на трошок. При клик на копчето **Delete** од
  темплејтот `list.html`, потребно е да се избрише трошокот од базата на податоци. Потоа треба да се прикаже
  страната `/expenses`.
    - Имплементацијата на оваа функционалност може да ја проверите со тестовите `test_delete_3pt`
      и `test_delete_mvc_2pt`.

- **(20 поени)** Потребно е да се имплементира промена на информации за трошокот. При клик на копчето **Edit** од
  темплејтот `list.html`, потребно е да се прикаже темплејтот `form.html` на патеката `/expenses/edit/[id]`, при што
  во `<input>` елементите ќе се прикажат вредностите за трошокот кој се менува. При клик на **Submit** треба да
  се запише промената на трошокот во базата на податоци. Потоа треба да се прикаже страната `/expenses`.
    - Имплементацијата на оваа функционалност може да ја проверите со тестовите `test_edit_10pt` и `test_edit_mvc_10pt`.

- **(5 поени)** Потребно е да се имплементира продолжување на бројот на денови до истекување на трошокот. При клик на
  копчето **Extend** од темплејтот `list.html`, потребно е да се зголеми бројот на денови до истекување за 1. Потоа
  треба да се прикаже страната `/expenses`.
...
- **(20 поени)** Потребно е да конфигурирате најава на `/login` и одјава на `/logout`. Притоа, јавни се патеките `/`
  и `/expenses`, додека сите останати страници треба да се видливи само за корисник со улога admin. Дополнително,
  кај `list.html` копчињата **Edit**, **Delete** и **Add new expense** треба да се видливи само за корисник кој има
...
- **(10 поени)** Потребно е да овозможите пребарување на трошоците по наслов (дали испратениот текст се
  содржи во променливата `title` за некој трошок), вендор и категорија преку формата со `id="filter-form"` во
  темплејтот `list.html`. Резултатите од пребарувањето треба да се прикажат на истата страница, при што ќе се прикажат
  само трошоци чиј наслов го содржат испратениот текст, се од избраниот вендор и се од соодветно
  избраната категорија. Филтрирањето се извршува само според внесените полиња (ако се празни, се игнорира филтрирањето
  по тој критериум).
...
- Во пакетот `mk.ukim.finki.wp.kol2025g3.model` веќе се креирани класите кои го репрезентираат моделот.
  ...
  Притоа важат следните услови:
    - Еден трошок е од точно еден вендор, додека пак еден вендор може да се однесува на повеќе трошоци.
    - Идентификаторите за `Expense` и `Vendor` треба да бидат генерирани.
- Во пакетот `mk.ukim.finki.wp.exam.kol2025g3.service` се веќе дефинирани интерфејсите за сервисната логика.
  За секој од методите имате опис што треба да биде имплементирано. Потребно е да се имплементираат овие интерфејси во
  соодветните класи во пакетот `mk.ukim.finki.wp.exam.kol2025g3.service.impl`. Во коментарите на методите се објаснети
  дополнителни услови (доколку ги има) кои треба да ги исполни методот.
- Класите од пакетот `mk.ukim.finki.wp.exam.kol2025g3.repository` треба да ги дополните со потребните методи кои ви се
...
- Во класата `mk.ukim.finki.wp.exam.kol2025g3.ExpensesController` се дефинирани сите методи кои се потребни.
