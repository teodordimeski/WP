...
- **(20 поени)** Потребно е да се имплементира додавање на трошок. При клик на копчето **Add new expense** од
  ...
  на **Submit** ќе се креира и запише нова трошок во базата на податоци. Потоа треба да се прикаже
  страната `/expenses`.
...
- **(20 поени)** Потребно е да се имплементира промена на информации за трошокот. При клик на копчето **Edit** од
  ...
  во `<input>` елементите ќе се прикажат вредностите за трошокот кој се променува. При клик на **Submit** треба да
...
- **(5 поени)** Потребно е да се имплементира продолжување на денови до истекување на трошокот. При клик на копчето *
  *Extend** од темплејтот `list.html`, потребно е да се зголеми бројот на денови до истекување за 1. Потоа треба да се
  прикаже страната `/expenses`.
...
- **(10 поени)** Потребно е да овозможите пребарување на трошоците по наслов (дали испратениот текст се
  содржи во променлицата `title` за некој трошок), вендор и категорија преку формата со `id="filter-form"` во
