...
//        commitSolution(index);
    ...
    public static void getAndLoadLastSolution(String index) throws IOException {
        ...
        ResponseEntity<String> response = restTemplate.exchange("http://185.153.49.149/eg/api/grading/" + examId + "/last_submission/" + index, HttpMethod.GET, new HttpEntity<>(createHeaders(System.getenv("WP_USER"), System.getenv("WP_PASS"))), String.class);
    ...
    public static void getSolutionById(Integer id) throws IOException {
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<String> response = restTemplate.exchange("http://185.153.49.149/eg/api/grading/submission/" + id, HttpMethod.GET, new HttpEntity<>(createHeaders(System.getenv("WP_USER"), System.getenv("WP_PASS"))), String.class);
        loadSolution(new ByteArrayInputStream(response.getBody().getBytes()));
    }
    ...
    private static Integer examId = 29987;
}
